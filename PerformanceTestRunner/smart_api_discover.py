...
class UrlMapSmartApiFetcher(object):
    ...
    def _irhits_from_res(self, j):
        for hit in j["hits"]:
            ...
            component = getpath(hit, ["info", "x-translator", "component"])
            ...
            if servers is not None:
                for server in servers:
                    ...
                    if x_trapi_version is not None:
                        if infores != 'infores:automat-icees-kg' and infores != 'infores:automat-robokop':
                            d = {
                                "infores": infores,
                                "urlServer": urlServer,
                                "maturity": maturity,
...
class SmartApiDiscoverer:
    ...
    def __init__(self, maturity):
        self._maturity = maturity #os.getenv("TR_ENV") if os.getenv("TR_ENV") is not None else "production"
    ...
    def urlServer(self, inforesid):
        ...
        return ["urlServer"] if "urlServer" in u else None
